    ...
    <div class="col mb-3" v-for="user in users">
      ...
      <div class="card card-custom-width">
        ...
        <div class="card-body">
          ...
          <router-link tag="button" class="btn btn-warning" :to="`/edit/${user.id}`">
...
export default {
  ...
  methods: {
    async getAllUsers() {
      ...
      let result = await axios.get(`${import.meta.env.VITE_API_URL}/users`);
    ...
    deleteItem(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          let result = await axios.delete(`${import.meta.env.VITE_API_URL}/users/${id}`);
